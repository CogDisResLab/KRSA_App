...
make_graphs <- function(controlSamplesTest, experimentalSamplesTest, fullDataSet, opt1, opt2, lower, upper){
  ...
  return(list(results_table, results_table_prime, before_qc))
}
...
fullDataSet[, 67:ncol(fullDataSet)] -> cont_df
fullDataSet[, 31:66] -> case_df

old_krsa_res <- make_graphs(cont_df, case_df, fullDataSet, T, T, 0.65,1.35)

setdiff(passed_qc1$V1, ppPassAll)


# new

source("scripts/reading_data.R")
new_df <- reading_data(fullDataSet)

new_df %>% filter(`Sample name` %in% c("M1", "M2", "M3", "F1", "F2", "F3")) -> filtered_samples

filtered_samples %>% filter(`Exposure time` == 200) %>%
  select(Peptide, `Sample name`, Signal) %>% 
  spread(key = `Sample name`, value = Signal) %>% View()
...
Dataset_PW_C1_Fit %>% filter(Peptide %in% ppPassAll) %>% 
  mutate(Group = ifelse(`Sample name` %in% c("M1", "M2", "M3"), "Control", "Case")) %>% 
...
setdiff(ppPassR2_avg,passed_qc2$V1)
...
grouped_fit %>% filter(Peptide %in% ppPassR2_avg) %>% 
  ...
  filter(FC <= 0.65 | FC >= 1.35) %>% pull(Peptide) %>% unique() -> sig_pep

setdiff(old_krsa_res[[1]]$V1,sig_pep)
